            ...
            <div class="flex-1 flex-shrink md:flex-shrink-0">
                ...
                <div class="mt-4 lg:mt-0">
                    <form action="/items/create/lost" method="POST" class="flex flex-col justify-center space-y-3 items-center sm:space-y-2">
                        ...
                        <x-form-input type="text" name="name" id="name" value="{{$item->name}}" required
                        ...
                        <x-form-input type="text" name="location" id="location" value="{{$item->location}}" required
                        class="text-xs sm:text-sm md:flex-shrink-0"></x-form-input>
                        <x-form-error name="location"></x-form-error>

                        <x-form-label class="ms-14 sm:ms-20 sm:text-sm lg:ps-14" >Description</x-form-label>
                        <textarea class="border border-gray-300 rounded-md p-2 w-2/3" name="description" id="description" value="{{$item->description}}"></textarea>
                        <x-form-error name="description"></x-form-error>

                        <div class="flex flex-wrap py-4">
                            <div class="flex flex-col items-center">
                                <x-form-label class="ps-16 lg:ps-0 text-md" >Date Lost</x-form-label>
                                <x-form-input type="date" name="date_lost_found" id="date_lost_found" value="{{$item->date_lost_found}}" required
                            ...
                            <div class="flex flex-col items-center">
                                ...
                                <x-form-input type="file" name="imgurl" id="imgurl" value="{{$item->imgurl}}" required
                                class="ms-8 lg:ms-0 text-xs sm:text-sm md:flex-shrink-0"></x-form-input>
                                <x-form-error name="imgurl"></x-form-error>
                            </div>                        
                        </div>

                        <select type="{{Auth::user()->role == 'user' ? 'hidden' : 'text'}}" name="status" value="{{$item->status}}">
                        ...
                        <x-form-input type="text" name="student_id" id="student_id" value="{{$item->student_id}}" required
                        class="text-xs sm:text-sm md:flex-shrink-0"></x-form-input>
                        ...
                        <x-form-input type="text" name="contact_number" id="contact_number" value="{{$item->contact_number}}" required
                        class="text-xs sm:text-sm md:flex-shrink-0"></x-form-input>
                        <x-form-error name="contact_number"></x-form-error>

                        <x-secondary-btn type="submit" class="self-end me-24">Save</x-secondary-btn>
                        <x-primary-btn formtype="a" href="/browse" class="self-end me-24">Cancel</x-primary-btn>
                    </form>
                </div>
            </div>
        </div>
